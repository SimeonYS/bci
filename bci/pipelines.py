...
class BbciPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bci.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bci`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bci WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bci (date, title, link, content)"
